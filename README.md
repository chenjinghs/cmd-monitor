...
cmd-monitor doctor         # 自检 daemon / IPC / Claude hooks
cmd-monitor hooks install  # 安装 Claude Code hooks
```

支持的 Claude 事件：`Notification`、`Stop`、`PreToolUse`

实测说明：普通文本追问/澄清在当前环境下通常只会落到 `Stop`，不会稳定触发 `Notification`；目前稳定观测到并可依赖的等待类事件是 `PreToolUse` (tool_name=AskUserQuestion)，以及结束态的 `Stop`。

## 多 tab 工作流程

1. `cmd-monitor start` 起 daemon(只保留唯一飞书 WebSocket 连接)
2. 在多个 WT tab 里跑 Claude Code
